...
export default function Concierge({ clubs, onSelect }: { clubs: Club[]; onSelect: (id: string) => void }) {
  ...
  return <div className="fixed bottom-5 right-5 z-[900] md:bottom-7 md:right-7">
    {open && <div className="mb-3 flex h-[470px] w-[min(360px,calc(100vw-32px))] flex-col overflow-hidden rounded-[24px] border border-[#dbe5df] bg-[#fffdf9] shadow-[0_20px_60px_rgba(23,52,59,.22)]">
      ...
      <div className="flex-1 overflow-y-auto p-4"><div className="mb-3 max-w-[85%] rounded-2xl rounded-tl-sm bg-[#edf2ed] p-3 text-sm text-[#3e5b5a]">Hi, I&apos;m Milo. Ask me to find a club by vibe, neighborhood, or occasion.</div>{messages.length === 0 && <div className="mb-4 flex flex-wrap gap-2">{['Best club for a romantic dinner?', 'Compare privacy and social energy', 'Plan my perfect Miami day'].map((prompt) => <button key={prompt} type="button" onClick={() => sendMessage({ text: prompt })} className="rounded-full border border-[#dbe5df] bg-white px-3 py-2 text-left text-[11px] font-medium text-[#668080] transition-colors hover:border-[#e8a28e] hover:text-[#d86f55]">{prompt}</button>)}</div>}{messages.map((message) => <div key={message.id} className={`mb-3 flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] rounded-2xl p-3 text-sm ${message.role === 'user' ? 'rounded-br-sm bg-[#e56f51] text-white' : 'rounded-bl-sm bg-[#edf2ed] text-[#3e5b5a]'}`}>{message.parts?.map((part, i) => part.type === 'text' ? <span key={i}>{part.text}</span> : null)}</div></div>)}{(status === 'submitted' || status === 'streaming') && <div aria-live="polite" className="mb-3 flex items-center gap-2 text-xs text-[#81938e]"><span className="flex gap-1 rounded-full bg-[#edf2ed] px-3 py-2"><span className="size-1.5 animate-bounce rounded-full bg-[#e56f51] [animation-delay:-.2s]" /><span className="size-1.5 animate-bounce rounded-full bg-[#e56f51] [animation-delay:-.1s]" /><span className="size-1.5 animate-bounce rounded-full bg-[#e56f51]" /></span><span>{status === 'submitted' ? 'Milo is thinking…' : 'Milo is replying from the club guide…'}</span></div>}</div>
      <form onSubmit={submit} className="flex gap-2 border-t border-[#e2e8e2] p-3"><input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Milo anything…" aria-label="Message Milo" disabled={status === 'submitted' || status === 'streaming'} className="min-w-0 flex-1 rounded-full border border-[#dbe5df] bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-[#e8a28e] disabled:cursor-wait disabled:opacity-60" /><Button type="submit" size="icon" className="size-9 rounded-full bg-[#17343b]" aria-label="Send" disabled={status === 'submitted' || status === 'streaming'}><Send size={15} /></Button></form>
    </div>}
